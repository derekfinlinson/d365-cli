...
function write (answers) {
    ...
    let templatePath = path.resolve(__dirname, 'templates', 'create');
    ...
    fs.writeFileSync(path.resolve(templatePath, 'creds.json'), JSON.stringify(credConfig));
...
function install() {
    // Run npm install
    const install = [
        answers.package === 'npm' ? 'install' : 'add',
        '@types/xrm',
        'node-webresource',
    ...
    if (answers.pack === 'npm') {
        spawn.sync('npm', install, { cwd: destinationPath, stdio: 'inherit' });
    } else {
        spawn.sync('yarn', install, { cwd: destinationPath, stdio: 'inherit' });
    }
}
