...
export async function deployStep(step: PluginStep, solution: string, apiConfig: WebApiConfig): Promise<string> {
  let stepId = await retrieveStep(step.name, apiConfig);
  const messageId = await getSdkMessageId(step.message, apiConfig);

  if (messageId == undefined) {
    console.error(`sdk message ${step.message} not found`);
    return;
  }
